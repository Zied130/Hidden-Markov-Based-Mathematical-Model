...
class HMM:
    def __init__(self, M,C,D,C1,C2,pi,SUFF,SUFF1,word2idx):
...
#        delta = np.zeros(self.M)
        delta1=np.zeros((T-1,self.M,self.M))
        psi = np.zeros((T,self.M, self.M))
#        delta[:] = np.log(self.pi) + np.log(self.B[:,x[0]])
        
        for j in range(self.M):
                for i in range(self.M):
                    try:
                        delta1[0,j,i]=np.log(self.pi[i])+np.log(self.C[j,i,x[1]])
                    except IndexError:
                        try:
                            delta1[0,j,i]=np.log(self.pi[i])+np.log(self.C2[j,i,SUFF.index([*word2idx][x[1]][:2])])
                        except ValueError:
                            try:
                                delta1[0,j,i]=np.log(self.pi[i])+np.log(self.C1[j,i,SUFF1.index([*word2idx][x[1]][:1])])
                            except ValueError:
                                delta1[0,j,i]=np.log(self.pi[i])
#        for j in range(0,M):
#            delta1[0,j,:]=delta[:]
        for t in range(1, T-1):
            for j in range(self.M):
                for i in range(self.M):
                    try:
                        delta1[t,j,i]=np.max(delta1[t-1,:,j]+np.log(self.D[:,j,i]))+np.log(self.C[j,i,x[t+1]])
                        psi[t,j,i]=np.argmax(delta1[t-1,:,j]+np.log(self.D[:,j,i]))
                    except IndexError:
                        try:
                            delta1[t,j,i]=np.max(delta1[t-1,:,j]+np.log(self.D[:,j,i]))+np.log(self.C2[j,i,SUFF.index([*word2idx][x[t+1]][:2])])
                            psi[t,j,i]=np.argmax(delta1[t-1,:,j]+np.log(self.D[:,j,i]))
                        except ValueError:
                            delta1[t,j,i]=np.max(delta1[t-1,:,j]+np.log(self.D[:,j,i]))+np.log(self.C1[j,i,SUFF1.index([*word2idx][x[t+1]][:1])])
                            psi[t,j,i]=np.argmax(delta1[t-1,:,j]+np.log(self.D[:,j,i]))
        # backtrack
        states = np.zeros(T, dtype=np.int32)
        states[T-1] = np.unravel_index(np.argmax(delta1[-1,:,:], axis=None), delta1[-1,:,:].shape)[1]
        states[T-2] = np.unravel_index(np.argmax(delta1[-1,:,:], axis=None), delta1[-1,:,:].shape)[0]
...
hmm = HMM(M,C,D,C1,C2,pi,SUFF,SUFF1,word2idx)
...
for line in open('test1_all.txt',encoding='utf-8'):
    ...
    if line:
        r = line.split()
        tag = r[2]
        currentX.append(pos2idx[tag])
        
    elif split_sequences:
            Ptruetest.append(currentX)
            currentX = []
            
for L in Ptruetest:
    L.insert(0,Ptruetest[0][-1])
...
print("test accuracy:", accuracy(Ptruetest, Ptrain))
accuracy=accuracy(Ptruetest, Ptrain)
print("test f1:", total_f1_score(Ptruetest, Ptrain))
f1=total_f1_score(Ptruetest, Ptrain)
print("test accuracy for unknown words:",accuracy_unknown(Ptruetest, Ptrain,Xtest))
unknown_ac=accuracy_unknown(Ptruetest, Ptrain,Xtest)
print("test accuracy for known words:",accuracy_known(Ptruetest, Ptrain,Xtest))
known_ac=accuracy_known(Ptruetest, Ptrain,Xtest)

Y = np.concatenate(Ytest)
P = np.concatenate(Ptruetest)
Z = np.concatenate(Ptrain)   
...
str_pythontex=[float("{0:.2f}".format(list(table_10.loc["A"])[0]*100)),float("{0:.2f}".format(list(table_10.loc["A"])[1]*100)),
float("{0:.2f}".format(list(table_10.loc["A"])[2]*100)),float("{0:.2f}".format(list(table_10.loc["A"])[4]*100)),
round(list(table_10.loc["A"])[3]),round(list(table_10.loc["A"])[5]),
float("{0:.2f}".format(list(table_10.loc["B"])[0]*100)),float("{0:.2f}".format(list(table_10.loc["B"])[1]*100)),
float("{0:.2f}".format(list(table_10.loc["B"])[2]*100)),float("{0:.2f}".format(list(table_10.loc["B"])[4]*100)),
round(list(table_10.loc["B"])[3]),round(list(table_10.loc["B"])[5]),
float("{0:.2f}".format(list(table_10.loc["C"])[0]*100)),float("{0:.2f}".format(list(table_10.loc["C"])[1]*100)),
float("{0:.2f}".format(list(table_10.loc["C"])[2]*100)),float("{0:.2f}".format(list(table_10.loc["C"])[4]*100)),
round(list(table_10.loc["C"])[3]),round(list(table_10.loc["C"])[5]),
float("{0:.2f}".format(list(table_10.loc["D"])[0]*100)),float("{0:.2f}".format(list(table_10.loc["D"])[1]*100)),
float("{0:.2f}".format(list(table_10.loc["D"])[2]*100)),float("{0:.2f}".format(list(table_10.loc["D"])[4]*100)),
round(list(table_10.loc["D"])[3]),round(list(table_10.loc["D"])[5]),
float("{0:.2f}".format(list(table_10.loc["E"])[0]*100)),float("{0:.2f}".format(list(table_10.loc["E"])[1]*100)),
float("{0:.2f}".format(list(table_10.loc["E"])[2]*100)),float("{0:.2f}".format(list(table_10.loc["E"])[4]*100)),
round(list(table_10.loc["E"])[3]),round(list(table_10.loc["E"])[5]),
float("{0:.2f}".format(list(table_10.loc["F"])[0]*100)),float("{0:.2f}".format(list(table_10.loc["F"])[1]*100)),
float("{0:.2f}".format(list(table_10.loc["F"])[2]*100)),float("{0:.2f}".format(list(table_10.loc["F"])[4]*100)),
round(list(table_10.loc["F"])[3]),round(list(table_10.loc["F"])[5]),
float("{0:.2f}".format(list(table_10.loc["G"])[0]*100)),float("{0:.2f}".format(list(table_10.loc["G"])[1]*100)),
float("{0:.2f}".format(list(table_10.loc["G"])[2]*100)),float("{0:.2f}".format(list(table_10.loc["G"])[4]*100)),
round(list(table_10.loc["G"])[3]),round(list(table_10.loc["G"])[5]),
float("{0:.2f}".format(list(table_10.loc["H"])[0]*100)),float("{0:.2f}".format(list(table_10.loc["H"])[1]*100)),
float("{0:.2f}".format(list(table_10.loc["H"])[2]*100)),float("{0:.2f}".format(list(table_10.loc["H"])[4]*100)),
round(list(table_10.loc["H"])[3]),round(list(table_10.loc["H"])[5]),
float("{0:.2f}".format(list(table_10.loc["I"])[0]*100)),float("{0:.2f}".format(list(table_10.loc["I"])[1]*100)),
float("{0:.2f}".format(list(table_10.loc["I"])[2]*100)),float("{0:.2f}".format(list(table_10.loc["I"])[4]*100)),
round(list(table_10.loc["I"])[3]),round(list(table_10.loc["I"])[5]),
float("{0:.2f}".format(list(table_10.loc["J"])[0]*100)),float("{0:.2f}".format(list(table_10.loc["J"])[1]*100)),
float("{0:.2f}".format(list(table_10.loc["J"])[2]*100)),float("{0:.2f}".format(list(table_10.loc["J"])[4]*100)),
round(list(table_10.loc["J"])[3]),round(list(table_10.loc["J"])[5]),
float("{0:.2f}".format(list(table_10.loc["K"])[0]*100)),float("{0:.2f}".format(list(table_10.loc["K"])[1]*100)),
float("{0:.2f}".format(list(table_10.loc["K"])[2]*100)),float("{0:.2f}".format(list(table_10.loc["K"])[4]*100)),
round(list(table_10.loc["K"])[3]),round(list(table_10.loc["K"])[5]),
float("{0:.2f}".format(list(table_10.loc["L"])[0]*100)),float("{0:.2f}".format(list(table_10.loc["L"])[1]*100)),
float("{0:.2f}".format(list(table_10.loc["L"])[2]*100)),float("{0:.2f}".format(list(table_10.loc["L"])[4]*100)),
round(list(table_10.loc["L"])[3]),round(list(table_10.loc["L"])[5]),
float("{0:.2f}".format(list(table_10.loc["M"])[0]*100)),float("{0:.2f}".format(list(table_10.loc["M"])[1]*100)),
float("{0:.2f}".format(list(table_10.loc["M"])[2]*100)),float("{0:.2f}".format(list(table_10.loc["M"])[4]*100)),
round(list(table_10.loc["M"])[3]),round(list(table_10.loc["M"])[5]),
float("{0:.2f}".format(list(table_10.loc["."])[0]*100)),float("{0:.2f}".format(list(table_10.loc["."])[1]*100)),
float("{0:.2f}".format(list(table_10.loc["."])[2]*100)),float("{0:.2f}".format(list(table_10.loc["."])[4]*100)),
round(list(table_10.loc["."])[3]),round(list(table_10.loc["."])[5]),
...
v10=[float("{0:.2f}".format(float(accuracy)*100)),float("{0:.2f}".format(float(f1)*100)),
float("{0:.2f}".format(float(unknown_ac)*100)),float("{0:.2f}".format(float(known_ac)*100)),
len(word2idx)-w_known,float("{0:.2f}".format(float((len(word2idx)-w_known)/len(word2idx))*100))]
